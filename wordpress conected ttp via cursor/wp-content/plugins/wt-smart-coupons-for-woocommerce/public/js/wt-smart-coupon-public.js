jQuery(function ($) {
    ...
    $('document').ready(function(){
        // If a popup/button still points users to checkout, redirect that button to cart.
        $(document).on('click', 'a, button', function(e){
            const text = $(this).text().trim().toLowerCase();
            if ( text.indexOf('proceed to checkout') !== -1 ) {
                const href = ( $(this).attr('href') || '' ).toLowerCase();
                if ( href.indexOf('checkout') !== -1 || '' === href || '#' === href ) {
                    e.preventDefault();
                    window.location.href = WTSmartCouponOBJ.cart_url;
                }
            }
        });
