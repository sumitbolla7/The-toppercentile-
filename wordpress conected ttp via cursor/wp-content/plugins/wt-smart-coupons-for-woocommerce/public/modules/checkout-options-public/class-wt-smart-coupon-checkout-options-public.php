	...
	public function wt_woocommerce_coupon_is_valid( $valid, $coupon ) {
		...
		if ( ! empty( $shipping_method_ids ) ) {
			...
			if ( ! empty( $chosen_shipping_methods ) ) {
				 ...
				 */
				$chosen_shipping = apply_filters( 'wt_sc_chosen_shipping_for_validation', $chosen_shipping, $chosen_shipping_methods ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound

				if ( ! in_array( $chosen_shipping, $shipping_method_ids, true ) ) {
					throw new Exception( esc_html__( 'Sorry, this coupon is not applicable to selected shipping method', 'thetoppercentile-coupons' ), 109 );
				}
			}
		}
		...
		if ( ! empty( $payment_method_ids ) ) {
			...
			if ( ! in_array( $chosen_payment_method, $payment_method_ids, true ) ) {
				throw new Exception( esc_html__( 'Sorry, this coupon is not applicable to selected Payment method', 'thetoppercentile-coupons' ), 109 );
			}
		}
		...
		if ( ! empty( $user_roles ) ) {
			...
			if ( empty( array_intersect( $user_roles, $current_user_roles ) ) ) {
				/**
				 * Filter to alter the user role validation message.
				 *
				 * @since 1.4.6
				 * @param string $message Message.
				 * @return string Message.
				 */
				$message = apply_filters( 'wt_sc_alter_user_role_validation_message', __( 'Sorry, this coupon is not applicable for your Role', 'thetoppercentile-coupons' ) ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
				throw new Exception( esc_html( $message ), 109 );
			}
		}
		...
		if ( ! empty( $available_locations ) && ! is_null( WC()->session ) ) {
			...
			if ( ! in_array( $choosed_location, $available_locations, true ) ) {
				throw new Exception( esc_html__( 'Sorry, this coupon is not applicable to selected location', 'thetoppercentile-coupons' ), 109 );
			}
		}
