...
if ( ! class_exists( 'Wt_Smart_Coupon_Restriction' ) ) {
	return;
}
...
class Wt_Smart_Coupon_Restriction_Public extends Wt_Smart_Coupon_Restriction {
	...
	/**
	 * BOGO applicable count session id
	 *
	 * @var string
	 */
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_coupon_is_valid', array( $this, 'wt_woocommerce_coupon_is_valid' ), 11, 2 );
		 ...
		 *  @since 1.4.1
		 */
		add_filter( 'woocommerce_coupon_is_valid_for_product', array( $this, 'exclude_disqualified_products' ), 10, 3 );
	}
	...
	private function process_qty_from_restriction_data( $item_id, $type, $wt_sc_items_data ) {
		...
		return ( 0 === $qty && 'min' === $type ? 1 : $qty );
	}

	/**
	 * Get individual min/max quantity validation message
	 *
	 * @since 1.4.0
	...
	public function get_individual_min_max_quantity_validation_message( $item_name, $qty, $coupon_code, $type = 'no_valid_products' ) {
		if ( 'min' === $type ) {
			...
			$msg = sprintf( __( 'The minimum quantity of %1$s for this coupon is %2$s.', 'thetoppercentile-coupons' ), $item_name, $qty );
		} elseif ( 'max' === $type ) {
			// translators: %1$s: Item name. %2$s: Quantity.
			$msg = sprintf( __( 'The maximum quantity of %1$s for this coupon is %2$s.', 'thetoppercentile-coupons' ), $item_name, $qty );
		} else {
			$msg = __( 'Your cart does not meet the quantity eligibility criteria for this coupon.', 'thetoppercentile-coupons' );
		}

		/**
		 * Filter to alter the individual min/max quantity validation message.
		 *
		 * @since 1.4.0
		...
		return apply_filters(
			'wt_sc_alter_individual_min_max_quantity_validation_message', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			$msg,
			array(
				'item_name'   => $item_name,
				'quantity'    => $qty,
	...
	/**
	 * Individual min/max quantity validation
	 *
	 ...
	 * @param int    $item_id Item id.
	 * @param array  $wt_sc_items_data Items data.
	 * @param array  $items_to_check_qty Items to check quantity.
	 * @param array  $items_to_check_name Items to check name.
	 * @param bool   $valid Valid.
	 * @param bool   $throw_exception Throw exception.
	...
	private function individual_min_max_quantity_validation( $coupon_code, $item_id, $wt_sc_items_data, $items_to_check_qty, $items_to_check_name, &$valid, $throw_exception = true ) {
		...
		if ( $min_qty > 0 && $items_to_check_qty[ $item_id ] < $min_qty ) {
			...
			if ( $throw_exception ) {
				throw new Exception(
					esc_html( $this->get_individual_min_max_quantity_validation_message( $items_to_check_name[ $item_id ], $min_qty, $coupon_code, 'min' ) ),
					110
				);
			}
		}
		...
		if ( $max_qty > 0 && $items_to_check_qty[ $item_id ] > $max_qty ) {
			...
			if ( $throw_exception ) {
				throw new Exception(
					esc_html( $this->get_individual_min_max_quantity_validation_message( $items_to_check_name[ $item_id ], $max_qty, $coupon_code, 'max' ) ),
					111
				);
			}
		}
	 ...
	 * @since    1.6.0       Now uses the common function `get_matching_product_property`
	 * @param object $coupon Coupon object.
	 * @param array  $coupon_products Coupon products.
	 * @param array  $coupon_categories Coupon categories.
	 * @param array  $coupon_exclude_products Coupon exclude products.
	 * @param array  $coupon_exclude_categories Coupon exclude categories.
	 * @param array  $args Arguments.
	 * @return   int         Total quantity of matching products
	 */
	 ...
	 *  @param array  $coupon_exclude_categories Coupon exclude categories.
	 *  @param array  $args Arguments.
	 *  @return float    Subtotal of matching products
	 */
	...
	private function prepare_term_name_for_validation_error_message( $category_id, &$items_to_check_name ) {
		$items_to_check_name[ $category_id ] = __( 'the category', 'thetoppercentile-coupons' );
	...
	public static function remove_bogo_applicable_count_session( $coupon_code ) {
		...
		if ( isset( $bogo_applicable_count[ $coupon_code ] ) && ! is_null( WC()->session ) ) {
	...
	public static function get_bogo_applicable_count_session() {
		return ( ! is_null( WC()->session ) ? WC()->session->get( self::$bogo_applicable_count_session_id ) : array() );
	}
	...
	/**
	 * Set applicable count by subtotal
	 *
	 * @param string $restriction_type Restriction type.
	 * @param string $coupon_code Coupon code.
	 * @param int    $min_subtotal Minimum subtotal.
	...
	/**
	 * Get applicable quantity by coupon and type
	 *
	 * @param string $coupon_code Coupon code.
	 * @param string $restriction_type Restriction type.
	 * @return array Applicable quantity.
	 */
	...
	/**
	 * Set applicable count by global quantity
	 *
	 * @param string $restriction_type Restriction type.
	 * @param string $coupon_code Coupon code.
	 * @param int    $item_quantity Item quantity.
	 * @param int    $wt_min_matching_product_qty Minimum matching product quantity.
	 */
	 ...
	 *
	 *  @param array $total_valid_arr Total valid array.
	 *  @param int   $item_quantity Item quantity.
	 *  @param int   $wt_min_matching_product_qty Minimum matching product quantity.
	 *  @return array Total valid array.
	 */
	...
	/**
	 * Set applicable count by quantity
	 *
	 * @param string $restriction_type Restriction type.
	 * @param string $coupon_code Coupon code.
	 * @param int    $product_id Product id.
	 * @param array  $items_to_check_qty Items to check quantity.
	 * @param array  $wt_sc_products_data Items data.
	 */
	...
	/**
	 * Prepare applicable count by quantity for and condition
	 *
	 * @param array $total_valid_arr Total valid array.
	 * @param int   $item_id Item id.
	 * @param array $wt_sc_items_data Items data.
	...
	/**
	 * Validate coupon usage restriction
	 *
	 *  @since 1.4.0
	...
	public function wt_woocommerce_coupon_is_valid( $valid, $coupon ) {
		...
		if ( is_admin() ) {
			...
			if ( is_null( $cart ) || empty( $cart->get_cart() ) ) {
				return $valid;
			}
		...
		if ( Wt_Smart_Coupon_Public::module_exists( 'giveaway-product-public' ) ) {
			$giveaway_obj = Wt_Smart_Coupon_Giveaway_Product_Public::get_instance();
		}

		if ( Wt_Smart_Coupon_Public::module_exists( 'bogo-public' ) ) {
			$bogo_obj = Wbte_Smart_Coupon_Bogo_Public::get_instance();
		}
		...
		if ( 'yes' === $wt_enable_product_category_restriction ) {
			...
			if ( 'and' === $wt_product_condition || 'or' === $wt_product_condition ) {
				...
				if ( count( $coupon_products ) > 0 ) {
					...
					foreach ( $items as $item ) {
						/* is free item check */
						if ( ( ! is_null( $giveaway_obj ) && $giveaway_obj->is_a_free_item( $item ) ) || ( ! is_null( $bogo_obj ) && $bogo_obj->is_a_free_item( $item ) ) ) {
							continue;
						}

						$product_name = ( is_object( $item['data'] ) && method_exists( $item['data'], 'get_name' ) ? ( "'" . $item['data']->get_name() . "'" ) : __( 'the product', 'thetoppercentile-coupons' ) );
					...
					if ( 'or' === $wt_product_condition ) {
						...
						foreach ( $coupon_products as $product_id ) {
							if ( in_array( $product_id, $items_to_check, true ) && isset( $wt_sc_products_data[ $product_id ] ) ) {
						...
						if ( 'yes' === $use_individual_min_max ) {
							if ( 0 === $valid_products ) {
								throw new Exception(
									esc_html( $this->get_individual_min_max_quantity_validation_message( '', 0, $coupon_code ) ),
									112
								);
							} else {
								$valid = true;
						...
						foreach ( $coupon_products as $product_id ) {
							if ( ! in_array( $product_id, $items_to_check, true ) ) {
							...
							} elseif ( 'yes' === $use_individual_min_max && isset( $wt_sc_products_data[ $product_id ] ) ) {
								// Product found, do quantity check for individual product.
					...
					if ( ! $valid ) {
						throw new Exception( esc_html__( 'Sorry, this coupon is not applicable for selected products.', 'thetoppercentile-coupons' ), 109 );
					}
				}
			}
			...
			if ( ! is_null( $giveaway_obj ) && ! $giveaway_obj->is_bogo( $coupon ) && ( 'and' === $wt_category_condition || 'or' === $wt_category_condition ) ) {
				$valid = true;
				...
				if ( count( $coupon_categories ) > 0 ) {
					...
					foreach ( $items as $item ) {
						...
						if ( ! empty( $coupon_excluded_categories ) ) {
							$product_excluded = array_intersect( $product_cats, $coupon_excluded_categories );
							if ( ! empty( $product_excluded ) ) {
								continue;
							}
						}

						$matching_cats = array_unique( array_intersect( $product_cats, $coupon_categories ) );
					...
					if ( $valid ) {
						...
						if ( 'or' === $wt_category_condition ) {
							...
							foreach ( $coupon_categories as $category_id ) {
								if ( in_array( $category_id, $items_to_check, true ) && isset( $categories_data[ $category_id ] ) ) {
									if ( 'yes' === $use_individual_min_max ) {
							...
							if ( 'yes' === $use_individual_min_max ) {
								if ( 0 === $valid_cats ) {
									$msg = $this->get_individual_min_max_quantity_validation_message( '', 0, $coupon_code );
									if ( '' !== $msg ) {
							...
							$total_valid_arr = array();
							foreach ( $coupon_categories as $category_id ) {
								if ( ! in_array( $category_id, $items_to_check, true ) ) {
									// clear coupon applicable session data.
									$this->remove_bogo_applicable_count_session( $coupon_code );

									$valid = false;
									break;
								} elseif ( 'yes' === $use_individual_min_max && isset( $categories_data[ $category_id ] ) ) {
									// Category found, do quantity check for individual category, prepare term name for error message.
									$this->prepare_term_name_for_validation_error_message( $category_id, $items_to_check_name );
					...
					if ( ! $valid ) {
						...
						throw new Exception( sprintf( esc_html__( 'Sorry, the coupon %s is not applicable for selected products.', 'thetoppercentile-coupons' ), wp_kses_post( "<b>{$coupon_code}</b>" ) ), 109 );
					}
				}
			}
			...
			if ( ! is_null( $giveaway_obj ) && $giveaway_obj->is_bogo( $coupon ) ) {
				$coupon_categories          = array();
				$coupon_excluded_categories = array();
			} else {
				$coupon_categories          = $coupon->get_product_categories();
				...
				foreach ( $items as $item ) {
					...
					if ( 'yes' === $use_individual_min_max ) {
						if ( $wt_min_matching_product_qty > 0 && $item['quantity'] < $wt_min_matching_product_qty ) {
							...
							$this->remove_bogo_applicable_count_session( $coupon_code );
							throw new Exception(
								esc_html( $this->get_quantity_restriction_messages( $coupon_code, $wt_min_matching_product_qty, false ) ),
								110
							);
						}
						...
						if ( $wt_max_matching_product_qty > 0 && $item['quantity'] > $wt_max_matching_product_qty ) {
							...
							$this->remove_bogo_applicable_count_session( $coupon_code );
							throw new Exception(
								esc_html( $this->get_quantity_restriction_messages( $coupon_code, $wt_max_matching_product_qty, false, 'max' ) ),
								111
							);
						}
		...
		$subtotal_of_matching_product = $this->get_subtotal_of_matching_products( $coupon, $coupon_products, $coupon_categories, $coupon_excluded_products, $coupon_excluded_categories );

		if ( $wt_min_matching_product_subtotal > 0 ) {
			if ( $subtotal_of_matching_product < $wt_min_matching_product_subtotal ) {
				if ( in_array( $coupon->get_code(), $woocommerce->cart->get_applied_coupons(), true ) ) {
					if ( $subtotal_of_matching_product < $wt_min_matching_product_subtotal ) {
						$valid = false;
						$this->remove_bogo_applicable_count_session( $coupon_code );
						throw new Exception(
							// translators: %s: Minimum subtotal.
							sprintf( esc_html__( 'The minimum subtotal of matching products for this coupon is %s.', 'thetoppercentile-coupons' ), wp_kses_post( Wt_Smart_Coupon_Admin::get_formatted_price( $wt_min_matching_product_subtotal ) ) ),
							112
						);
					}
				} else {
		...
		if ( $wt_max_matching_product_subtotal > 0 && $subtotal_of_matching_product > $wt_max_matching_product_subtotal ) {
			...
			$this->remove_bogo_applicable_count_session( $coupon_code );
			throw new Exception(
				// translators: %s: Maximum subtotal.
				sprintf( esc_html__( 'The maximum subtotal of matching products for this coupon is %s.', 'thetoppercentile-coupons' ), wp_kses_post( Wt_Smart_Coupon_Admin::get_formatted_price( $wt_max_matching_product_subtotal ) ) ),
				113
			);
		}
		
		$valid = $this->is_cart_valid_for_brand_settings( $valid, $coupon );
	...
	/**
	 * Validate min/max global quantity
	 *
	 * @param string $coupon_code Coupon code.
	 * @param bool   $valid Valid.
	 * @param int    $quantity_of_matching_product Quantity of matching product.
	 * @param int    $wt_min_matching_product_qty Minimum matching product quantity.
	 * @param int    $wt_max_matching_product_qty Maximum matching product quantity.
	 * @return bool Valid.
	 * @throws Exception When coupon is not valid.
	 */
	private function validate_min_max_global_qty( $coupon_code, $valid, $quantity_of_matching_product, $wt_min_matching_product_qty, $wt_max_matching_product_qty ) {
		if ( $wt_min_matching_product_qty > 0 && $quantity_of_matching_product < $wt_min_matching_product_qty ) {
			...
			throw new Exception(
				esc_html( $this->get_quantity_restriction_messages( $coupon_code, $wt_min_matching_product_qty, true ) ),
				110
			);
		}
		if ( $wt_max_matching_product_qty > 0 && $quantity_of_matching_product > $wt_max_matching_product_qty ) {
			...
			throw new Exception(
				esc_html( $this->get_quantity_restriction_messages( $coupon_code, $wt_max_matching_product_qty, true, 'max' ) ),
				111
			);
		}
		return $valid;
	 ...
	 *
	 *  @since    1.4.1
	 *  @since    1.4.4     [Bug fix] When multiple coupons with same product restriction and different quantitiy restriction occurs, items in disqualified array is checked for individual coupon code instead of one single array to avoid the product quantity eligibility issues.
	 ...
	 *  @param object $coupon Coupon object.
	 */
	public function exclude_disqualified_products( $valid, $product, $coupon ) {
		$coupon_code = wc_format_coupon_code( $coupon->get_code() );

		if ( isset( $this->disqualified[ $coupon_code ] ) && ! empty( $this->disqualified[ $coupon_code ] ) ) {
			$this->recheck_disqualified_products( $coupon->get_id() );
	...
	/**
	 * Get quantity restriction messages
	 *
	 * @param string $coupon_code Coupon code.
	 * @param int    $qty Quantity.
	...
	private function get_quantity_restriction_messages( $coupon_code, $qty, $is_global, $type = 'min' ) {
		...
		if ( 'min' === $type ) {
			if ( $is_global ) {
				...
				$out = sprintf( __( 'Coupon applies to %s quantity. Please move eligible number of products in the cart to redeem the coupon.', 'thetoppercentile-coupons' ), $qty );
			} else {
				// translators: %s: Quantity.
				$out = sprintf( __( 'Each eligible cart item requires minimum %s quantity, please add more items to cart to redeem the coupon.', 'thetoppercentile-coupons' ), $qty );
			}
		} elseif ( $is_global ) {
			// translators: %s: Quantity.
			$out = sprintf( __( 'The maximum quantity of matching products for this coupon is %s.', 'thetoppercentile-coupons' ), $qty );
		} else {
			// translators: %s: Quantity.
			$out = sprintf( __( 'The maximum allowed quantity for each eligible item is %s', 'thetoppercentile-coupons' ), $qty );
		}

		/**
		 * Filter to alter the quantity restriction messages.
		 *
		 * @since 1.4.0
		 * @param string $out Message.
		 * @param string $coupon_code Coupon code.
		 * @param int $qty Quantity.
		 * @param bool $is_global Is global.
		 * @param string $type Type.
		 * @return string Message.
		 */
		return apply_filters( 'wt_sc_alter_quantity_restriction_messages', $out, $coupon_code, $qty, $is_global, $type ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
	}
	...
	private function is_order_edit_page() {
		$basename = isset( $_SERVER['PHP_SELF'] ) ? basename( wp_parse_url( sanitize_text_field( wp_unslash( $_SERVER['PHP_SELF'] ) ), PHP_URL_PATH ) ) : '';
		...
		if ( 'post.php' === $basename ) {
			...
			if ( 'woocommerce_add_coupon_discount' === $action ) {
				return isset( $_POST['order_id'] ) ? absint( wp_unslash( $_POST['order_id'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Missing
			}
		}
	...
	public function get_matching_product_property( $coupon, $args ) {
		...
		foreach ( $items as $item_key => $item ) {
			...
			if ( ! isset( $matching_items[ $item_key ] ) ) {
				// product restriction.
				if ( $is_product_restriction_enabled ) {
					if ( in_array( $item['product_id'], $coupon_products, true ) || in_array( $item['variation_id'], $coupon_products, true ) ) {
				...
				if ( ! $is_product_restriction_enabled && ! $is_category_restriction_enabled ) {
					if ( ! empty( $coupon_exclude_categories ) || ! empty( $coupon_exclude_products ) ) {
						...
						if (
							! in_array( $item['product_id'], $coupon_exclude_products, true )
							&& ! in_array( $item['variation_id'], $coupon_exclude_products, true )
	...
	/**
     * Recheck disqualified products.
     *
     * @since 2.2.5
     *
     * @param int $coupon_id     Coupon ID.
     */
    private function recheck_disqualified_products( $coupon_id ) {
        $use_individual_min_max = $this->get_coupon_meta_value( $coupon_id, '_wt_use_individual_min_max' );
        $coupon_code = wc_format_coupon_code( wc_get_coupon_code_by_id( $coupon_id ) );

        if ( 'yes' !== $use_individual_min_max || ! isset( $this->disqualified[ $coupon_code ] ) || empty( $this->disqualified[ $coupon_code ] ) ) {
            return;
        }
        
        $coupon             = new WC_Coupon( $coupon_id );
        $disqualified_items = $this->disqualified[ $coupon_code ];
        ...
        $items_to_check_qty_products   = array();
        $items_to_check_qty_categories = array();
        ...
        foreach ( self::$items as $item ) {
            ...
            $variation_id = isset( $item['variation_id'] ) && $item['variation_id'] > 0 ? $item['variation_id'] : 0;
            
            if ( in_array( $product_id, $coupon_products, true ) || in_array( $variation_id, $coupon_products, true ) ) {
                $items_to_check_qty_products[ $product_id ] = ( isset( $items_to_check_qty_products[ $product_id ] ) ? $items_to_check_qty_products[ $product_id ] + $item['quantity'] : $item['quantity'] );
                if ( $variation_id > 0 ) {
                    $items_to_check_qty_products[ $variation_id ] = ( isset( $items_to_check_qty_products[ $variation_id ] ) ? $items_to_check_qty_products[ $variation_id ] + $item['quantity'] : $item['quantity'] );
                }
            }
            ...
            if ( ! empty( $coupon_categories ) ) {
                ...
                foreach ( $matching_cats as $category_id ) {
                    $items_to_check_qty_categories[ $category_id ] = ( isset( $items_to_check_qty_categories[ $category_id ] ) ? $items_to_check_qty_categories[ $category_id ] + $item['quantity'] : $item['quantity'] );
                }
            }
        }
        
        foreach ( $disqualified_items as $key => $disqualified_item_id ) {
            $is_valid_now = false;
            
            if ( in_array( $disqualified_item_id, $coupon_products, true ) && isset( $wt_sc_products_data[ $disqualified_item_id ] ) && isset( $items_to_check_qty_products[ $disqualified_item_id ] ) ) {
                $item_qty = $items_to_check_qty_products[ $disqualified_item_id ];
                $min_qty = $this->process_qty_from_restriction_data( $disqualified_item_id, 'min', $wt_sc_products_data );
                $max_qty = $this->process_qty_from_restriction_data( $disqualified_item_id, 'max', $wt_sc_products_data );
                $is_valid_now = ( $min_qty == 0 || $item_qty >= $min_qty ) && ( $max_qty == 0 || $item_qty <= $max_qty );
            } elseif ( in_array( $disqualified_item_id, $coupon_categories, true ) && isset( $categories_data[ $disqualified_item_id ] ) && isset( $items_to_check_qty_categories[ $disqualified_item_id ] ) ) {
                $category_qty = $items_to_check_qty_categories[ $disqualified_item_id ];
                $min_qty = $this->process_qty_from_restriction_data( $disqualified_item_id, 'min', $categories_data );
                $max_qty = $this->process_qty_from_restriction_data( $disqualified_item_id, 'max', $categories_data );
                $is_valid_now = ( $min_qty == 0 || $category_qty >= $min_qty ) && ( $max_qty == 0 || $category_qty <= $max_qty );
            }
            
            if ( $is_valid_now ) {
                unset( $this->disqualified[ $coupon_code ][ $key ] );
            }
        }
        
        if ( empty( $this->disqualified[ $coupon_code ] ) ) {
            unset( $this->disqualified[ $coupon_code ] );
        } else {
            $this->disqualified[ $coupon_code ] = array_values( $this->disqualified[ $coupon_code ] );
        }
    }
	...
	/**
	 * Checks if the cart is valid for brand.
	 *
	 * @since 2.2.5
	 *
	 * @param bool $valid Valid.
	 * @param WC_Coupon $coupon Coupon object.
	 * @return bool Valid true if the cart is valid for brand, false otherwise.
	 */
	private static function is_cart_valid_for_brand_settings( $valid, $coupon ) {
		if ( ! $valid || ! class_exists( 'WC_Brands_Brand_Settings_Manager' ) || ! method_exists( 'WC_Brands_Brand_Settings_Manager', 'get_brand_settings_on_coupon' ) ) {
			return $valid;
		}

		$items = self::$items;

		$brand_restriction = WC_Brands_Brand_Settings_Manager::get_brand_settings_on_coupon( $coupon );

		if ( empty( $brand_restriction['included_brands'] ) ) {
			return $valid;
		}
		...
		foreach ( $items as $item ) {
			...
			if ( ! empty( array_intersect( $product_brands, $brand_restriction['included_brands'] ) ) ) {
				$valid = true;
				break;
			}
		}
		...
		/**
		 * If any third-party plugin checks brands like SC, it will cause conflict, so that can be altered by this hook.
		 * @since 2.2.5
		 * @param bool $valid Valid.
		 * @param WC_Coupon $coupon Coupon object.
		 * @return bool Valid.
		 */
		$valid = apply_filters( 'wbte_sc_alter_cart_valid_for_brand', $valid, $coupon );

		return $valid;
	}

	/**
	 * Checks if the item is a free item.
	 *
	 * @since 2.2.5
	 *
	 * @param array $item Item.
	 * @return bool True if the item is a free item, false otherwise.
	 */
	private static function is_a_free_item( $item ) {
		$giveaway_obj = null;
		$bogo_obj     = null;
		if ( Wt_Smart_Coupon_Public::module_exists( 'giveaway-product-public' ) ) {
			$giveaway_obj = Wt_Smart_Coupon_Giveaway_Product_Public::get_instance();
		...
		if ( ! is_null( $bogo_obj ) && $bogo_obj->is_a_free_item( $item ) ) {
			return true;
		}
		return false;
	}
