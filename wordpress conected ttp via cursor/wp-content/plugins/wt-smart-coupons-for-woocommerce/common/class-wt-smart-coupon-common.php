...
if ( ! class_exists( 'Wt_Smart_Coupon_Common' ) ) {
	...
	class Wt_Smart_Coupon_Common {
		...
		public static $modules = array(
			...
			'giveaway-product',
			'restriction',
			'checkout-options', /* @since 1.4.6 */
			'style', /* @since 1.4.7 */
			'auto-coupon-common', /* @since 1.7.0 */
			'exclude-product-common', /* @since 2.0.0 */
		);
		...
		/**
		 *  Lookup table allowed meta keys.
		 *
		 *  @var array
		 */
		public static $lookup_table_allowed_meta_keys = array(
			'_wt_make_auto_coupon'          => array( 'is_auto_coupon', '%d' ),
			'_wbte_sc_auto_coupon_priority' => array( 'auto_coupon_priority', '%d' ), /* @since 1.7.0 */
		 ...
		 * @return Wt_Smart_Coupon_Common Instance of the class.
		 */
		 ...
		 *
		 *  @since  1.4.1
		 *  @param  WC_DateTime|string|int $date  Date value.
		 ...
		 *  @param  bool        $gmt   return GMT timestamp (optional). Default:true.
		 ...
		 *  @return boolean True if coupon exists, false otherwise.
		 */
		public static function is_coupon_exists( $coupon ) {
			global $wpdb;
			if ( ! is_null( $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type ='shop_coupon' AND post_status = 'publish' AND post_title = %s ", $coupon ) ) ) ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching 
				return true;
			}
			return false;
		}
		...
		public function update_existing_coupon_data_to_lookup_table() {
			 ...
			 *  @since 1.4.4
			 */
			$batch_limit = apply_filters( 'wt_sc_lookup_table_migration_batch_limit', 100 ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound

			// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Only used when activating the plugin, so no need to cache.
			...
			// remove duplicates.
			$wpdb->query( "DELETE t1 FROM {$lookup_tb} t1 INNER JOIN {$lookup_tb} t2 WHERE t1.id < t2.id AND t1.coupon_id = t2.coupon_id" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter
		 ...
		 *
		 *  @since  1.4.3
		 *  @param  WC_Coupon $data_obj   Coupon object.
		 */
		public function update_coupon_lookup_on_object_save( $data_obj ) {
		 ...
		 */
		public function update_coupon_lookup_on_meta_save( $post_id ) {
		 ...
		 *  @param  int       $new_count   New usage count.
		 */
		public function update_coupon_lookup_on_usage_count_change( $coupon, $new_count ) {
			$post_id = $coupon->get_id();

			if ( $this->is_no_lookup_table_entry( $post_id ) ) {
				...
				if ( Wt_Smart_Coupon::is_table_exists( $lookup_tb ) ) {
					$wpdb->update( $lookup_tb, array( 'usage_count' => $new_count ), array( 'coupon_id' => $post_id ), array( '%d' ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
				}
			}
		}
		 ...
		 */
		public function update_coupon_lookup_on_postmeta_change( $meta_id, $object_id, $meta_key ) {
			if ( in_array( $meta_key, array_keys( self::$lookup_table_allowed_meta_keys ), true ) && 'shop_coupon' === get_post_type( $object_id ) ) {
		...
		public function update_coupon_lookup_on_post_status_change( $new_status, $old_status, $post ) {
			if ( 'shop_coupon' === get_post_type( $post ) ) {
				...
				if ( $this->is_no_lookup_table_entry( $post_id ) ) {
					...
					if ( Wt_Smart_Coupon::is_table_exists( $lookup_tb ) ) {
						$wpdb->update( $lookup_tb, array( 'post_status' => $new_status ), array( 'coupon_id' => $post_id ), array( '%s' ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
					}
				}
			}
		}
		...
		private function update_data_to_lookup_table( $data_row, $insert = false, $meta_key = '' ) {
			...
			if ( $insert && ! $this->is_no_lookup_table_entry( $coupon_id ) ) {
				return;
			}
			 ...
			 */
			if ( ! $insert && '' !== $meta_key && isset( self::$lookup_table_allowed_meta_keys[ $meta_key ] ) ) {
				if ( '_wc_make_coupon_available' === $meta_key ) {
					$coupon_loc  = $this->get_meta_data_for_lookup_table( $coupon, $coupon_id, $meta_key );
					$update_data = array(
						'my_account_display' => absint( in_array( 'my_account', $coupon_loc, true ) ),
						'cart_display'       => absint( in_array( 'cart', $coupon_loc, true ) ),
						'checkout_display'   => absint( in_array( 'checkout', $coupon_loc, true ) ),
					);
				...
				$wpdb->update( $lookup_tb, $update_data, array( 'coupon_id' => $coupon_id ), $update_data_format, array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			...
			if ( $insert ) {
				if ( $this->is_no_lookup_table_entry( $coupon_id ) ) {
					$wpdb->insert( $lookup_tb, $data_arr, $data_format_arr ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
				}
			} else {
				$wpdb->update( $lookup_tb, $data_arr, array( 'coupon_id' => $coupon_id ), $data_format_arr, array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
			}
		}

		/**
		 * Get meta data for lookup table
		 *
		 * @since 1.4.3
		 *
		 * @param WC_Coupon $coupon Coupon object.
		...
		private function get_meta_data_for_lookup_table( $coupon, $coupon_id, $meta_key ) {
			...
			switch ( $meta_key ) {
				...
				case 'date_expires':
					...
					if ( true === $coupon_expiry_days_enabled && 0 < $coupon_expiry_days ) {
					...
					$out = ( $coupon_expiry ? gmdate( 'Y-m-d', $coupon_expiry ) : '' );
					break;
		...
		private function check_and_update_coupon_lookup_table( $post_id, $force_insert = false, $meta_key = '' ) {
			...
			$post_row = $wpdb->get_row( $wpdb->prepare( "SELECT ID, post_status, post_date_gmt FROM {$wpdb->posts} WHERE ID=%d", absint( $post_id ) ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		private function is_no_lookup_table_entry( $post_id ) {
			...
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT coupon_id FROM {$lookup_tb} WHERE coupon_id = %d", $post_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
			return empty( $row );
		}


		/**
		 * Check and update lookup table
		 *
		 * @since 1.4.4
		 */
		public function check_and_update_lookup_table() {
			if ( Wt_Smart_Coupon::get_lookup_table_version() > Wt_Smart_Coupon::get_installed_lookup_table_version() ) {
		 ...
		 *  @return array      Processed order items, structure similar to cart items.
		 */
		public static function convert_order_item_like_cart_item( $order_items ) {
			...
			/**
			 * Filter hook to alter converted order items
			 *
			 * @since 1.4.4
			 *
			 * @param array $new_cart_items Processed order items, structure similar to cart items.
			 * @param array $order_items Order items array.
			 */
			return apply_filters( 'wt_sc_alter_converted_order_items', $new_cart_items, $order_items ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		}
		...
		public static function get_lookup_table_record_count() {
			...
			$row = $wpdb->get_row( "SELECT COUNT(DISTINCT coupon_id) AS total_records FROM {$lookup_tb}", ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter
		...
		public static function get_product_cat_ids( $product_id ) {
			/**
			 * Filter hook to alter product categories with ancestors
			 *
			 * @since 1.6.0
			 *
			 * @param bool $with_ancestors Whether to include product categories with ancestors.
			 * @param int $product_id Product ID.
			 */
			if ( apply_filters( 'wt_sc_product_categories_with_ancestors', false, $product_id ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		 ...
		 */
		public static function coupon_delete_from_lookup_table_when_deleted( $post_id ) {
			if ( 'shop_coupon' === get_post_type( $post_id ) ) {
				global $wpdb;
				$lookup_tb = Wt_Smart_Coupon::get_lookup_table_name();

				if ( Wt_Smart_Coupon::is_table_exists( $lookup_tb ) ) {
					$wpdb->delete( $lookup_tb, array( 'coupon_id' => $post_id ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching
				}
			}
		}
		...
		public static function bogo_restricted_pages() {
			...
			/**
			 * Filter hook to later revamp BOGO restricted pages
			 *
			 * @since 2.0.0
			 *
			 * @param array $restricted_pages Array of BOGO restricted pages.
			 * @return array Array of BOGO restricted pages.
			 */
