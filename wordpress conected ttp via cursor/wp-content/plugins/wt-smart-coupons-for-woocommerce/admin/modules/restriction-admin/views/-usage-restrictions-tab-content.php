<?php
/**
 * Usage restrictions tab content
 *
 * @link
 * @since 1.4.0
 *
	...
	<table class="wt_sc_coupon_meta_item_table" id="wt_sc_coupon_products">
		<thead>
			<tr>
				...
				<th><?php esc_html_e( 'Min. Quantity', 'thetoppercentile-coupons' ); ?></th>
				<th><?php esc_html_e( 'Max. Quantity', 'thetoppercentile-coupons' ); ?></th>
				<th></th>
			</tr>
		</thead>
		<tbody>
			...
			foreach ( $products_data as $product_id => $product_data ) {
				...
				<tr>
					<td class="wt_sc_meta_item_tb_item">
						<select class="wt_sc_product_search wt_sc_select2" data-default-val="" name="_wt_sc_coupon_product_ids[<?php echo esc_attr( $field_index ); ?>]" data-placeholder="<?php esc_attr_e( 'Search for a product...', 'thetoppercentile-coupons' ); ?>">
						<?php
						if ( '--' !== $product_id ) {
					...
					<td class="wt_sc_meta_item_tb_action">
						<span class="dashicons dashicons-dismiss wt_sc_meta_item_tb_delete_row" title="<?php esc_html_e( 'Remove row', 'thetoppercentile-coupons' ); ?>"></span>
					</td>
				</tr>
				<?php
				++$field_index;
			...
			?>
				...
				<td colspan="4" class="wt_sc_add_new_row_btn_td">
					<button type="button" class="wt_sc_meta_item_tb_add_row" title="<?php esc_html_e( 'Add new row', 'thetoppercentile-coupons' ); ?>">+</button>
				</td>
			</tr>
		</tbody>
	</table>  
...
<fieldset class="form-field wt_sc_coupon_categories_fieldset wt_sc_coupon_fieldset">
	<legend><?php esc_html_e( 'Products categories', 'thetoppercentile-coupons' ); ?></legend>
	<table class="wt_sc_coupon_meta_item_table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Category', 'thetoppercentile-coupons' ); ?></th>
				<th><?php esc_html_e( 'Min. Quantity', 'thetoppercentile-coupons' ); ?></th>
				<th><?php esc_html_e( 'Max. Quantity', 'thetoppercentile-coupons' ); ?></th>
				<th></th>
			</tr>
		...
		<tbody>
			...
			foreach ( $categories_data as $category_id => $category_data ) {
				...
				<tr>
					<td class="wt_sc_meta_item_tb_item">
						<select class="wt_sc_category_search wt_sc_select2" data-default-val="" name="_wt_sc_coupon_category_ids[<?php echo esc_attr( $field_index ); ?>]" data-placeholder="<?php esc_attr_e( 'Search for a category...', 'thetoppercentile-coupons' ); ?>">
						<?php
						if ( '--' !== $category_id ) {
