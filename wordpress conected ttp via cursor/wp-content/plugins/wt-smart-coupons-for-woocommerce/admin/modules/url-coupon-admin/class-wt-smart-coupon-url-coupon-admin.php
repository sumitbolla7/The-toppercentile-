...
class Wt_Smart_Coupon_Url_Coupon_Admin {
	...
	public function coupon_edit_page_section() {
		...
		if ( ( 'post' === $screen->base && 'shop_coupon' === $screen->post_type ) || ( isset( $_GET['page'] ) && WT_SC_PLUGIN_NAME === $_GET['page'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended 
			?>
			<style type="text/css">
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function(){
					...
					if(0==copy_to_clipboard.length && post_title_input.length>0)
					{
						...
						post_title_input.after('<span class="wt_sc_copy_to_clipboard" data-target="wt_sc_url_preview"><?php echo esc_html( __( 'Copy coupon URL', 'thetoppercentile-coupons' ) ); ?><span class="dashicons dashicons-admin-page"></span></span>');
					}

					var generate_coupon_code=jQuery('a.generate-coupon-code');
					if(generate_coupon_code.length>0)
					{
						generate_coupon_code.after('<span class="wt_sc_coupon_url_help_popup_link"><?php echo esc_html( __( 'How to use coupon URL?', 'thetoppercentile-coupons' ) ); ?></span>');
					}
			...
			<div class="wt_sc_url_coupon_help wt_sc_popup" style="width:850px;">
				<div class="wt_sc_popup_hd">
					<div class="wt_sc_popup_title"><?php esc_html_e( 'How to use coupon URL', 'thetoppercentile-coupons' ); ?></div>
	...
	public function settings_tabhead( $arr ) {
		...
		foreach ( $arr as $k => $v ) {
			$out_arr[ $k ] = $v;
			if ( 'wt-sc-layouts' === $k && 0 === $added ) {
				$out_arr[ 'wt-sc-' . $this->module_base ] = __( 'URL coupon', 'thetoppercentile-coupons' );
				$added                                    = 1;
			}
		}
