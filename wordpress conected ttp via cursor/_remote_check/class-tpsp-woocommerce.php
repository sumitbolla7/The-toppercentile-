...
class TPSP_WooCommerce {
    ...
    public function hooks() {
        add_action('template_redirect', [$this, 'force_guest_account_path_to_login'], 1);
        add_action('template_redirect', [$this, 'restrict_unverified_access'], 5);
        add_action('template_redirect', [$this, 'redirect_logged_out_my_account_to_login'], 5);
        ...
        add_filter('gettext', [$this, 'simplify_my_account_labels'], 20, 3);
    }
    ...
    public function force_guest_account_path_to_login() {
        ...
        $login_url  = $login_page instanceof WP_Post ? get_permalink($login_page) : home_url('/login/');
        if (empty($login_url)) {
            return;
        }
    ...
    /**
     * Simplify WooCommerce account page helper labels for students.
     *
     ...
     * @return string
     */
    public function simplify_my_account_labels($translated_text, $text, $domain) {
        if (!is_account_page() || 'woocommerce' !== $domain) {
            return $translated_text;
        }

        $replacements = [
            'A link to set a new password will be sent to your email address.' => 'We will send a password setup link to your email.',
            'Your personal data will be used to support your experience throughout this website, to manage access to your account, and for other purposes described in our %s.' => 'We use your details only to create and manage your account, as explained in our %s.',
